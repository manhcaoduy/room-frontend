...
import useCollision, { CollisionProps } from "./useCollision";
import useLastPosition, { LastPositionProps } from "./useLastPosition";
...
const useMovement = ({ object, floor }: MovementProps): void => {
  ...
  const moveForward = (distance: number): void => {
  ...
  const moveRight = (distance: number): void => {
    movementVector.setFromMatrixColumn(object.matrix, 0);
    object.position.addScaledVector(movementVector, distance);
  };
};
